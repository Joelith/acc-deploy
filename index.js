...
program
	...
	.option('-s, --status', 'Get the status of the application. Don\'t upload')
	.parse(process.argv);
...
if (program.status) {
	...
	rest.get(api_url + '/paas/service/apaas/api/v1.1/apps/' + identity_domain + '/' + program.name, {
		...
		console.error(chalk.red('Unable to get details from Applciation Container Cloud. Check your configuration details'));
				...
				// The application does not exist. Ask if they want us to create it for them
					...
					var data_for_create = {
						...
						rest.post(api_url + '/paas/service/apaas/api/v1.1/apps/' + identity_domain , {
							...
							console.log('Application creation is underway.');
							console.log('Please wait for several minutes before you access  \'' + program.name + '\'. The  url will be ' + result.data.appURL);
							resolve();
						},function(error) {
